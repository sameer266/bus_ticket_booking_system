...
class UserOtp(models.Model):
    ...
    def send_sms(to, otp):
        try:
            token = settings.SPARROW_TOKEN
            identity = settings.SPARROW_SENDER 
            text=f"Go Sewa OTP. Your otp is {otp}"
            
            url = f'https://api.sparrowsms.com/v2/sms/?token={token}&to={to}&from={identity}&text={text}'
